...
{
    public static class DeserializerStateExtensions
    {
        // Backporting a convenience method from Capnp.Net.Runtime that wasn't ever published
        public static bool IsStructFieldNonNull(this DeserializerState self, int index)
        {
            if (self.Kind != ObjectKind.Struct && self.Kind != ObjectKind.Nil)
            {
                throw new DeserializationException("This is not a struct");
            }

            if (index < 0 || index >= self.StructPtrCount)
            {
                throw new IndexOutOfRangeException($"Invalid index {index}. Must be [0, {self.StructPtrCount}).");
            }

            var pointerOffset = index + self.StructDataCount;
            WirePointer pointer = self.CurrentSegment[self.Offset + pointerOffset];
